...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        print("- Sʋccɘssʆʋɭɭƴ Joɩŋɘɗ Tʜɘ Vc.")
        await record_and_send_opus()
        await gc.stop()
    else:
        print("- Sʋccɘssʆʋɭɭƴ Lɘʆt Tʜɘ Vc.")
...
async def record_and_send_opus():
    ...
    status = await client.send_message(chat_id, "1/3 Rɘcoʀɗɩŋʛ......")
    ...
    await status.edit_text("2/3 Tʀʌŋscoɗɩŋʛ...")
    ffmpeg.input(
        record_raw,
        format='s16le',
        acodec='pcm_s16le',
    ...
    await status.edit_text("3/3 Upɭoʌɗɩŋʛ...")
